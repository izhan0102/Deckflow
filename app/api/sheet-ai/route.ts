...
- For ANY calculation (totals, sums, averages, growth, etc.) emit a FORMULA string, never a pre-computed number. The engine recomputes live. Supported: =SUM(A1:A9), =AVERAGE(B2:B10), =MIN, =MAX, =COUNT, =COUNTA, =PRODUCT, =ROUND(x,2), =ABS, =SQRT, =POWER(x,y), =FLOOR, =CEILING, =IF(cond,a,b), =CONCAT(...) (or the & operator for text), =RANDBETWEEN(min,max), =RAND(), and operators + - * / ^ % with parentheses and comparisons = <> < > <= >=. RANDBETWEEN is stable per cell, so it's fine for "realistic" sample data — OR just write varied literal numbers.
